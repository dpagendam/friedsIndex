...
credibleIntervals = function(samples, wildToSterileRatio = 1, col = "blue", quantiles = c(0.025, 0.25, 0.5, 0.75, 0.975))
{
	...
	par(mfrow = c(2,2))
	m = max(d1$y)
	plot(d1, ylim = c(0, m*1.25), xlab = "Estimate", col = "white", main = "Hatch Probability (Sterile Mated)", ylab = "Density")
	polygon(p1$x, p1$y, col = friedsIndex::fade(col, 75))
	
	m = max(d2$y)
	plot(d2, ylim = c(0, m*1.25), xlab = "Estimate", col = "white", main = "Hatch Probability (Wildtype Mated)", ylab = "Density")
	polygon(p2$x, p2$y, col = friedsIndex::fade(col, 75))
	
	m = max(d3$y)
	plot(d3, ylim = c(0, m*1.25), xlab = "Estimate", col = "white", main = "Proportion of Sterile Mated Females", ylab = "Density")
	...
	cat("-----------------------------------\n")
	cat("Quantiles and Mean for Hatch Probability (Sterile Mated)\n")
	...
	cat("-----------------------------------\n")
	cat("Quantiles and Mean for Hatch Probability (Wildtype Mated)\n")
	...
	cat("-----------------------------------\n")
	cat("Quantiles and Mean for Proportion of Sterile Mated Females\n")
